...
function InitLeftMenu(basePath) {
	...
	$.each(_menus.menus, function(i, n) {
		...
		$.each(n.child, function(j, o) {
			menulist += '<li><div><a ref="' + o.menuid
					+ '" href="javascript:void(0);" rel="' + basePath+o.url
					+ '" ><span class="icon ' + o.icon
					+ '" >&nbsp;</span><span class="nav">' + o.menuname
					+ '</span>'
					if (o.child && o.child.length > 0) {
						menulist +='<span class="arrow">&nbsp;</span>'
					}
			menulist +='</a></div> ';
			if (o.child && o.child.length > 0) {
				...
				$.each(o.child, function(k, p) {
					menulist += '<li><div><a ref="' + p.menuid
							+ '" href="javascript:void(0);" rel="' + basePath+p.url
	...
	$('.navlist li a').click(function() {
		...
		if (third && third.child && third.child.length > 0) {
			...
			if (ul.is(":hidden")){
				ul.slideDown();
				$(this).parent().parent().siblings().children('div').find('span:last').removeClass("down");
				$(this).parent().parent().parent().parent().parent().siblings().find('.navlist li div a').children('.arrow').removeClass("down");
			}else{
